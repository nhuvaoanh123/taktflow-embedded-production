// Taktflow Systems - ICU application (lifecycle manager, tasks, idle handler)

#include "app/IcuApp.h"
...
::etl::typed_storage<::taktflow::icu::systems::IcuCanSystem> icuCanSystem;
::etl::typed_storage<::taktflow::systems::TransportSystem> transportSystem;
::etl::typed_storage<::taktflow::icu::systems::DoCanSystem> doCanSystem;
::etl::typed_storage<::taktflow::icu::systems::IcuSystem> icuSystem;
::etl::typed_storage<::taktflow::icu::systems::UdsSystem> udsSystem;
...
private:
    void execute() override
    {
        ...
        {
            printf("ICU lifecycle shutdown complete\n");
            std::_Exit(0);
        }
        else
        {
...
void startApp()
{
    ...
    lifecycleManager.addComponent("can", icuCanSystem.create(TASK_CAN), 3u);

    // Level 4: Transport message router
    lifecycleManager.addComponent("transport", transportSystem.create(TASK_UDS), 4u);

    // Level 5: DoCAN ISO-TP + ICU application SWCs
    lifecycleManager.addComponent(
        "docan",
        doCanSystem.create(
            *transportSystem, icuCanSystem->getTransceiver(), TASK_UDS),
        5u);
    lifecycleManager.addComponent(
        "icu", icuSystem.create(TASK_ICU, icuCanSystem->getTransceiver()), 5u);
...
using IcuTask = AsyncAdapter::Task<TASK_ICU, 1024 * 2>;
IcuTask icuTask{"icu"};
