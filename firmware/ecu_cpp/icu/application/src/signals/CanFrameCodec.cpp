...
CanFrameCodec::CanFrameCodec(IcuSignals& signals, ::can::ICanTransceiver& transceiver)
: _signals(signals)
...
void CanFrameCodec::decodeRx(::can::CANFrame const& frame)
{
    ...
    switch (id)
    {
        ...
        case 0x011u: // FZC_Heartbeat
            _signals.fzcAliveCounter = data[2];
            break;

        case 0x012u: // RZC_Heartbeat
            _signals.rzcAliveCounter = data[2];
            break;
        ...
        case 0x301u: // Motor_Current
            _signals.overcurrentFlag = (data[2] >> 4) & 0x01u;
            break;
...
void CanFrameCodec::encodeTxHeartbeat()
{
    ...
    // byte 2: alive counter, byte 3: ECU ID
    frameData[2] = _signals.txAliveCounter;
    frameData[3] = HEARTBEAT_ECU_ID;

    // E2E protect (writes dataId, aliveCounter nibbles, CRC into bytes 0-1)
