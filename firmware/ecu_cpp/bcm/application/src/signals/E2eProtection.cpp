// Taktflow Systems - E2E protection implementation
...
uint8_t E2eProtection::computeCrc8(uint8_t const* data, uint8_t len)
{
    uint8_t crc = 0xFFu;
...
void E2eProtection::protect(
    uint8_t* payload, uint8_t dlc, uint8_t dataId, uint8_t aliveCounter)
{
    // E2E header in byte 0: bits 0-3 = dataId, bits 4-7 = aliveCounter
    payload[0] = static_cast<uint8_t>((dataId & 0x0Fu) | ((aliveCounter & 0x0Fu) << 4u));

    // CRC over byte 0 + bytes 2..dlc-1 (skip byte 1 which holds CRC)
    uint8_t crc = 0xFFu;
    crc = CRC8_TABLE[crc ^ payload[0]];
    for (uint8_t i = 2u; i < dlc; ++i)
    {
...
bool E2eProtection::validate(uint8_t const* payload, uint8_t dlc)
{
    if (dlc < 2u)
    {
        return false;
