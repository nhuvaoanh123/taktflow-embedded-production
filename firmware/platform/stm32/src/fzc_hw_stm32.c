/**
 * @file    fzc_hw_stm32.c
 * @brief   STM32 hardware backend for FZC (Front Zone Controller)
 * @date    2026-03-03
 *
 * @details Phase F2: PLL 170 MHz, bare-metal USART2 debug TX, SysTick via
 *          HAL, CAN loopback self-test wiring. Sensor and actuator stubs
 *          still return E_OK (real implementations come in F3-F4).
 *
 *          Clock: HSI 16 MHz → PLL (M/4 × N85 / R2) = 170 MHz
 *          UART:  Bare-metal USART2 PA2=TX AF7, 115200 baud @ 170 MHz
 *
 *          FZC UART note: USART2/PA2 is used for TFMini-S lidar in
 *          production, but Nucleo SB63/SB65 solder bridges route PA2
 *          to ST-LINK VCP. Fine for F2 bring-up. SB removal deferred
 *          to Phase F5 (sensors).
 *
...
/**
 * @brief  Steering servo neutral self-test — stub returns E_OK
 * @return E_OK
 * @note   TODO:HARDWARE — drive servo to neutral and read feedback
 */
Std_ReturnType Main_Hw_ServoNeutralTest(void)
{
    /* TODO:HARDWARE — real servo neutral test */
    return E_OK;
}

/**
 * @brief  SPI sensor self-test — stub returns E_OK
 * @return E_OK
 * @note   TODO:HARDWARE — read WHO_AM_I register via HAL_SPI
 */
Std_ReturnType Main_Hw_SpiSensorTest(void)
{
    /* TODO:HARDWARE — real SPI sensor test */
    return E_OK;
}

/**
 * @brief  UART lidar self-test — stub returns E_OK
 * @return E_OK
 * @note   TODO:HARDWARE — send probe command and check response
 */
Std_ReturnType Main_Hw_UartLidarTest(void)
{
    /* TODO:HARDWARE — real UART lidar test */
    return E_OK;
}
